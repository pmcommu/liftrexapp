...
export default function CreateProposal({ navigation ,route}) {
...
useEffect(() => {
  ...
  setTotalStops(String(d.total_amount_of_stops || ""));

  setOperation(d.operation || "");
  setCapacity(String(d.capacity || ""));
  setSpeedOfTravel(String(d.speed_of_travel || ""));
...
const getElevatorType = (formLabel = "") => {
  ...
  if (formLabel.startsWith("T")) return "Traction";
  if (formLabel.startsWith("H")) return "Hydraulic";
  return "";
  };
...
console.log('fulltype',getElevatorType(selectedForm))
...
const validate = () => {
  ...
  if (!elevatorName.trim())
    newErrors.elevatorName = "Elevator name is required";
  ...
  // if (hoistWidthFt === 0 && hoistWidthIn === 0)
  //   newErrors.hoistWidth = "Hoistway width is required";

  if (hoistDepthFt === 0 && hoistDepthIn === 0)
    newErrors.hoistDepth = "Hoistway depth is required";

  // if (doorFt === 0 && doorIn === 0)
  //   newErrors.doorSize = "Door size is required";

  // if (cabHeightFt === 0 && cabHeightIn === 0)
  //   newErrors.cabHeight = "Cab height is required";

  if (!hoistwayEntrance)
    newErrors.hoistwayEntrance = "Hoistway entrance is required";

  if (!doorSelection)
    newErrors.doorSelection = "Door selection is required";

  if (!doorSpeed) {
  newErrors.doorSpeed = "Door speed is required";
}
...
const buildPayload = () => ({
  ...
  elevator_type: getElevatorType(selectedForm),
  operation,

  capacity: toNumber(capacity),
  speed_of_travel: toNumber(speedOfTravel),
  total_amount_of_stops: toNumber(totalStops),
  ...
  door_size: toNumber(doorFt),
  door_size_in_inches: toNumber(doorIn),

  cab_size_feet: toNumber(cabHeightFt),
  cab_size_inches: toNumber(cabHeightIn),

  front_openings: Math.max(0, toNumber(frontOpening)),
  rear_openings: Math.max(0, toNumber(rearOpening)),
  side_openings: Math.max(0, toNumber(sideOpening)),
  ...
  feet_in_inches: toNumber(totalTravelIn), // ✅ FIXED
...
const handleSubmit = async () => {
  ...
  try {
    ...
    Toast.show({
      type: "success",
      text1: "Success",
      text2: "Calculation completed successfully",
    });
