...
export default function UpdatePaymentPhases({ navigation }) {
...
const buildPaymentPayload = () => {
  return {
    ...
    paymentId: project?.proposalDetails?.payment_id, 
...
const submitPaymentTerms = async () => {
  ...
  try {
    ...
    if (res?.data?.statusCode === 200) {
      ...
      // 🔥 Redux / local state update (agar hai)
      // dispatch(updatePaymentDetails(updatedPayment));

      Toast.show({
        type: "success",
        text1: "Payment Terms Updated",
        text2: "Changes saved successfully",
      });

      // 🚀 Optional navigation (same UX as proposal)
      setTimeout(() => {
        navigation.goBack();
        // OR
...
const formatAmount = (value) => {
  ...
  return (
    <View style={styles.container}>
      {/* HEADER */}
      <CustomHeader title="Update Payment Terms"
       onBackPress={() => {
